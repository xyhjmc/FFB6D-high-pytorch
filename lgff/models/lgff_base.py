...
class LGFFBase(nn.Module):
    """Core LGFF architecture without class-embedding heads."""
    ...
    def forward(self, inputs, end_points=None, scale=1):
        ...
        for i_ds in range(4):
            ...
            rgb_emb = self.ds_fuse_p2r_fuse_layers[i_ds](torch.cat((rgb_emb0, p2r_emb), dim=1))
            ...
            p_emb = self.ds_fuse_r2p_fuse_layers[i_ds](torch.cat((p_emb0, r2p_emb), dim=1))
            ds_emb.append(p_emb)
        ...
        for i_up in range(n_up_layers - 1):
            ...
            p2r_emb = p2r_emb.view(bs, -1, hr, wr)
            rgb_emb = self.up_fuse_p2r_fuse_layers[i_up](torch.cat((rgb_emb0, p2r_emb), dim=1))
        ...
        p_emb = self.rndla_up_stages[n_up_layers - 1](torch.cat([ds_emb[0], f_interp_i], dim=1)).squeeze(-1)
        ...
        pred_kp_ofs = pred_kp_ofs.view(bs, self.n_kps, 3, -1).permute(0, 1, 3, 2).contiguous()
