...
def depth_normal(
    ...
    try:
        ...
        # CUDA cannot be safely initialized inside forked DataLoader workers. Fallback to CPU to
        # avoid crashing distributed training when normal estimation runs in worker processes.
        if "Cannot re-initialize CUDA in forked subprocess" in str(err):
            warnings.warn(
                "Falling back to CPU normal estimation because CUDA is unavailable in this "
                "DataLoader worker; set num_workers=0 if GPU normals are required."
            )
